...
object KanaHelper {
    ...
    fun convertKana(input: String?): String {
        ...
        when {
            KanaHelper.isHiragana(ch) -> // convert to hiragana to katakana
                for (i in 0 until input.length) {
                    out.append(KanaHelper.toKatakana(input[i]))
                }
            KanaHelper.isKatakana(ch) -> // convert to katakana to hiragana
                for (i in 0 until input.length) {
                    out.append(KanaHelper.toHiragana(input[i]))
                }
